...
$(strip \
  $(if $(2),$(2), \
    $(if $(filter @ZyXEL_SITE/% @GNOME/% @GNU/% @KERNEL/% @SF/% ftp://% http://% https://% file://%,$(1)),default, \
      $(if $(filter git://%,$(1)),git, \
        $(if $(filter svn://%,$(1)),svn, \
          $(if $(filter cvs://%,$(1)),cvs, \
            $(if $(filter hg://%,$(1)),hg, \
...
# for check out package(compressed file) from SVN server
define DownloadMethod/svn_pkg
	@echo "Checking out $(FILE) from the svn repository..."; \
	rm -f $(DL_DIR)/$(FILE); mkdir -p $(DL_DIR) && cd $(DL_DIR) && \
	svn help export | grep -q trust-server-cert && \
        svn export --non-interactive --trust-server-cert -r$(VERSION) $(URL)/$(FILE) || \
        svn export --non-interactive -r$(VERSION) $(URL)/$(FILE); 
endef

# for check out GUI from SVN server
define DownloadMethod/svn_gui
	@echo "Checking out $(FILE) from the svn repository..."; \
	mkdir -p $(TOPDIR) && cd $(TOPDIR) && \
	svn help checkout | grep -q trust-server-cert && \
        svn checkout --non-interactive --trust-server-cert -r$(VERSION) $(URL)/$(FILE) || \
        svn checkout --non-interactive -r$(VERSION) $(URL)/$(FILE); 
endef
...
Validate/svn_pkg=VERSION
Validate/svn_gui=VERSION
