...
CUSTOM_KCONFIG_PREFIX=$(call qstrip, $(CONFIG_CUSTOM_KCONFIG_PREFIX))
...
ifeq ($(KERNEL),2.6)
GENERIC_FILES_DIR := $(foreach dir,$(wildcard $(GENERIC_PLATFORM_DIR)/files-2.6 $(GENERIC_PLATFORM_DIR)/files-$(KERNEL_PATCHVER)),"$(dir)")
else
ifeq ($(KERNEL),4.4)
GENERIC_FILES_DIR :=
else
...
LINUX_TARGET_CONFIG = $(call find_kernel_config,$(PLATFORM_DIR),$(CUSTOM_KCONFIG_PREFIX))
ifneq ($(PLATFORM_DIR),$(PLATFORM_SUBDIR))
  LINUX_SUBTARGET_CONFIG = $(call find_kernel_config,$(PLATFORM_SUBDIR),$(CUSTOM_KCONFIG_PREFIX))
endif
...
ifeq ("$(strip ${BOARD})","ipq")
  DEFAULT_CFLAGS_arm=-Os -pipe -march=armv7-a -mtune=cortex-a7
else
...
ifeq ("$(strip ${BOARD})","ipq")
  DEFAULT_CFLAGS=-Os -pipe -march=armv7-a -mtune=cortex-a7
endif
endif
