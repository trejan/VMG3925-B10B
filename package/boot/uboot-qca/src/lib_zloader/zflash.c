...
int znand_init(void)
{
    /* Do nothing since it's already initialized */
    return 0;
}
int znand_read(unsigned long offs, unsigned long len, char *buf)
{
    return nand_read_skip_bad(&nand_info[0], offs, &len,NULL,nand_info[0].size, (u_char *)buf);
//			if (nand_read_skip_bad(&nand_info[nand_env_device], offset,&len, NULL,  nand_info[nand_env_device].size, char_ptr))
} 

int znand_write(unsigned long offs, unsigned long len, char *buf)
{
    return nand_write_skip_bad(&nand_info[0], offs, &len,NULL,nand_info[0].size, (u_char *)buf,0);
}
...
#elif defined(CONFIG_CMD_NAND)
    return znand_read(offs, len, buf);
#endif
    return 0;
}
...
#elif defined(CONFIG_CMD_NAND)
    return znand_write(offs, len, buf);
#endif
    return 0;
}
