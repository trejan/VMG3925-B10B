...
static mtd_extra_info_t mtd_extra_info[CONFIG_SYS_MAX_NAND_DEVICE];

/* ath nand info */
typedef struct {
	/* mtd info */
	struct mtd_info		*mtd;
	mtd_extra_info_t	*mtd_extra;
			...
			uint32_t *addr1, int small_block_erase)
{
	...
	mtd_extra_info_t	*mtd_extra = sc->mtd_extra;
	...
	if (is_small_block_device(sc) && small_block_erase) {
		...
		// Get the block no.
		uint32_t b = (addr >> mtd_extra->erasesize_shift);
		...
		*addr1 = 0;
		 ...
		 * +-----+---+---+---+---+---+---+---+---+
		 */
		*addr0 = ((addr >> mtd_extra->writesize_shift) << 16);
		*addr1 = ((addr >> (mtd_extra->writesize_shift + 16)) & 0xf);
	}
}
...
#if ATH_NF_HW_ECC
	if (ecc_needed && sc->ecc_offset && (count & sc->mtd_extra->writesize_mask) == 0) {
		/*
...
ath_nand_dump_buf(loff_t addr, void *v, unsigned count)
{
	...
	for (; buf && buf < end; buf += 4, addr += 16) {
		printk("%08lx: %08x %08x %08x %08x\n",
			(unsigned long)addr, buf[0], buf[1], buf[2], buf[3]);
	}
		...
		loff_t addr, size_t len, size_t *iodone)
{
	...
	*iodone = 0;
	...
	while (len) {
		...
		c = (addr & mtd_extra->writesize_mask);
...
ath_nand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	s_first = instr->addr;
	n = instr->len >> mtd_extra->erasesize_shift;

	if (instr->len & mtd_extra->erasesize_mask) n ++;

	indbg("0x%llx 0x%x 0x%lx", instr->addr, n, mtd->erasesize);
	...
	for (j = 0, i = s_first; j < n; j++, i += mtd->erasesize) {
		...
		if ((ret = ath_nand_block_erase(sc, ba0, ba1)) != 0) {
			printf("%s: erase failed 0x%llx 0x%x 0x%x %lu "
		...
		struct mtd_oob_ops *ops)
{
	...
	pa = (void *)virt_to_phys(ath_nand_io_buf);
	if (!rd) flush_cache((ulong)ath_nand_io_buf, mtd->writesize + mtd->oobsize);	// for writes...
	...
	if (rd) {
		memcpy(ath_nand_io_buf, (void *)KSEG1ADDR(pa), mtd->writesize + mtd->oobsize);	// for reads...
...
static int ath_nand_read_oob(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	int ret;

        /* Do not allow reads past end of device */
        if (ops->datbuf && (from + ops->len) > mtd->size) {
                printf("%s: Attempt %s beyond end of device\n", __func__,"read");
                return -EINVAL;
        }
	
	if (ops->mode != MTD_OOB_RAW) {
		printf("%s: Only support MTD_OOB_RAW mode!\n", __func__);
                return -EINVAL;
	}

	if (ops->len > mtd->writesize) {
		printf("%s: Only support %s one page at one time!\n", __func__,"read");
                return -EINVAL;
	}

	ret = ath_nand_rw_oob(mtd, 1 /* read */, from, ops);
	if (ret) {
                printf("ath_nand_read_oob: "
			"Error (%d) reading offset %08x\n", ret, from);
                return -1;
        }
	
	if (ops->len!=0 && ops->datbuf!=NULL)
		memcpy(ops->datbuf+mtd->writesize, ops->oobbuf, mtd->oobsize);

	return 0;
}

static int ath_nand_write_oob(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
	int ret;
	ops->retlen = 0;
...
ath_nand_hw_init(ath_nand_sc_t *sc, void *p)
{
	...
	if (p) {
		...
		if ((rddata & ATH_NF_RD_STATUS_MASK) != ATH_NF_STATUS_OK) {
			iodbg("%s: ath nand status = 0x%x\n", __func__, rddata);
		}

		pa = p;
		iodbg("Ath Nand ID[%p]: %02x:%02x:%02x:%02x:%02x\n",
...
static ulong ath_nand_probe(void)
{
	...
	mtd_extra_info_t *mtd_extra = NULL;
	...
	sc->mtd_extra = &mtd_extra_info[nand_curr_device];
	...
	mtd = sc->mtd;
	mtd_extra = sc->mtd_extra;
	mtd->size = (uint64_t)ath_parse_read_id(sc) << 20;
	...
	if (is_small_block_device(sc)) {
		...
		mtd_extra->writesize_shift	= ffs(mtd->writesize) - 1;
		mtd_extra->writesize_mask	= mtd->writesize - 1;

		mtd->erasesize			= sc->entry->blk;
		mtd_extra->erasesize_shift	= ffs(mtd->erasesize) - 1;
		mtd_extra->erasesize_mask	= mtd->erasesize - 1;

		mtd->oobsize			= sc->entry->spare;
		mtd->oobavail			= mtd->oobsize;
	} else if (!sc->onfi[0]) {
		mtd_extra->writesize_shift	= 10 + sc->nid.ps;
		mtd->writesize			= (1 << mtd_extra->writesize_shift);
		mtd_extra->writesize_mask	= (mtd->writesize - 1);

		mtd_extra->erasesize_shift	= 16 + sc->nid.bs;
		mtd->erasesize			= (1 << mtd_extra->erasesize_shift);
		mtd_extra->erasesize_mask	= (mtd->erasesize - 1);
	...
	bbt_size = ((mtd->size >> mtd_extra->erasesize_shift) * 2) / 8;
