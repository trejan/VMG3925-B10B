...
#ifdef CONFIG_RUBY_SPI_FLASH_LOCK_MECHANISM
static u8 *g_spi_lock = NULL;
#endif

struct ruby_spi_flash {
	struct spi_flash flash;
	const struct flash_info *params;
};

static inline struct ruby_spi_flash *to_ruby_spi_flash(struct spi_flash *flash)
{
        return container_of(flash, struct ruby_spi_flash, flash);
}
...
static unsigned long spi_flash_uboot_size(void)
{
	 ...
	 */
	if (flash_size < UBOOT_TEXT_PARTITION_SIZE) {
		return SPI_FLASH_MINI_UBOOT_SIZE;
	}

	return UBOOT_TEXT_PARTITION_SIZE;
}
...
static void _spi_flash_probe(void)
{
	int i;
	u32 jedec_id;
...
static int _spi_flash_erase(u32 flash_addr, u32 size)
{
	...
	if (!ret) {
		...
		/* printf("_spi_flash_erase: erase size=%u, flash_size=%u, flash_addr=0x%08X\n", \
			   size, spi_flash_size(), flash_addr); */
...
static int _spi_flash_read(u32 flash_addr, u8 *mem_addr, u32 size, u32 *ret_size)
{
	int ret = spi_flash_read_op_check(flash_addr, size);

	if (!ret) {
		/* printf("_spi_flash_read: mem_addr=%p, read size=%u, flash_size=%u, flash_addr=0x%08X\n", \
			   mem_addr, size, spi_flash_size(), flash_addr); */
...
static int _spi_flash_write(u32 flash_addr, u8 *mem_addr, u32 size, u32 *ret_size)
{
	...
	if (!ret) {
		...
		/* printf("_spi_flash_write: mem_addr=%p, write size=%u, flash_size=%u, flash_addr=0x%08X\n", \
			   mem_addr, size, spi_flash_size(), flash_addr); */
...
//--- export SPI flash API ---
static int ruby_spi_flash_write(struct spi_flash *flash, u32 offset, size_t len, const void *buf)
{
	u32 ret_size;
	return _spi_flash_write(offset, (u8 *)buf, len, &ret_size);
}

static int ruby_spi_flash_erase(struct spi_flash *flash, u32 offset, size_t len)
{
	return _spi_flash_erase(offset, len);
}

static int ruby_spi_flash_read(struct spi_flash *flash, u32 offset, size_t len, void *buf)
{
	u32 ret_size;
	return _spi_flash_read(offset, (u8 *)buf, len, &ret_size);
}

struct spi_flash *spi_flash_probe(unsigned int bus, unsigned int cs,
                unsigned int max_hz, unsigned int spi_mode)
{
	struct ruby_spi_flash *new;

	if (!g_spi_flash) {
		_spi_flash_probe();
	}
    if ( g_spi_flash ) {
        new = (struct ruby_spi_flash *)malloc(sizeof(struct ruby_spi_flash));
        if ( new ) {
            new->params = g_spi_flash;
            new->flash.spi = NULL;
            new->flash.name = g_spi_flash->name;
            new->flash.write = ruby_spi_flash_write;
            new->flash.erase = ruby_spi_flash_erase;
            new->flash.read = ruby_spi_flash_read;
            new->flash.size = spi_flash_size();
#ifdef CONFIG_CMD_JFFS2
            new->flash.sector_size =  g_spi_flash->sector_size;
#endif
            return &new->flash;
        } else {
            puts("spi_flash_probe: memory not enough!\n");
        }	
    } else {
        puts("spi_flash_probe: not found spi flash!\n");
    }
    return NULL;
}
...
{
	struct ruby_spi_flash *new = to_ruby_spi_flash(flash);
	free(new);
}
//--------------------------------------

#if defined(CONFIG_RUBY_SPI_FLASH_CMD)
...
static int spi_flash_test_cmp(u32 flash_addr, u32 size, u8 val)
{
	...
	for (i = flash_addr; i < size; i += sizeof(read_buf)) {
		...
		read_ret = _spi_flash_read(i, read_buf, read_size, &ret_size);
...
static int spi_flash_test_set(u32 flash_addr, u32 size, u8 val)
{
	...
	for (i = flash_addr; i < size; i += SPI_PAGE_SIZE) {
		...
		write_ret = _spi_flash_write(i, page_buf, SPI_PAGE_SIZE, &ret_size);
...
static int spi_flash_test_speed(void)
{
	...
	for (i = 0; i < size; i += SPI_PAGE_SIZE) {
		...
		ret = _spi_flash_read(i, read_buf, read_size, &ret_size);
		if (ret) {
			printf("Read failed: %d\n", ret);
			goto done;
		}
...
static int spi_flash_test(void)
{
	...
	ret = _spi_flash_erase(0x0, spi_flash_size());
	if (ret) {
	...
	while(i < g_spi_flash->n_sectors) {
		...
		ret = _spi_flash_erase(i * g_spi_flash->sector_size, num * g_spi_flash->sector_size);
		if (ret) {
	...
	for (i = 0; i < 20; ++i) {
		...
		if (size) {
			...
			ret = _spi_flash_erase(sector_addr, 2 * g_spi_flash->sector_size);
			if (ret) {
...
int do_spi_flash(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (argc < 2) {
		...
		if (strcmp(cmd, "test") == 0) {
			...
			ret = _spi_flash_erase(flash_addr, size);
			...
			if (strcmp(cmd, "read") == 0) {
				printf("Read: ");
				ret = _spi_flash_read(flash_addr, mem_addr, size, &ret_size);
			} else if (strcmp(cmd, "write") == 0) {
				printf("Write: ");
				ret = _spi_flash_write(flash_addr, mem_addr, size, &ret_size);
			} else {
...
#if defined(CONFIG_RUBY_SPI_FLASH_CMD) && defined(CONFIG_SHOW_BOOT_PROGRESS)
	(void)spi_flash_info();
#endif
}
...
int saveenv(void)
{
	...
	ret = _spi_flash_write( flash_addr, (u8 *)env_ptr, size, &ret_size );
	if ( ret ) {
...
void env_relocate_spec (void)
{
	...
	_spi_flash_read( flash_addr, (u8 *)env_ptr, CONFIG_ENV_SIZE, &len);
}
...
int  env_init(void)
{
	...
	if (gd->env_valid == 0) {
		...
		/* read old CRC */
		_spi_flash_read( flash_addr, (u8 *)&crc, len, &ret_len);
		new = 0;
		len = ENV_SIZE;
		...
		while (len > 0) {
			...
			_spi_flash_read( flash_addr+off, buf, n, &ret_len);
			new = crc32 (new, buf, n);
			len -= n;
			off += n;
		}
