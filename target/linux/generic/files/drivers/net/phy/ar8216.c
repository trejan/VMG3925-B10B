...
#define SWITCH_PROC_ENTRY_PHY_REG_ADDR  "phyRegAddr"
#define SWITCH_PROC_ENTRY_PHY_REG_VAL   "phyRegVal"
#define SWITCH_PROC_ENTRY_CPU_REG_ADDR  "cpuRegAddr"
#define SWITCH_PROC_ENTRY_CPU_REG_VAL   "cpuRegVal"
...
static int switch_proc_phyAddr=1, switch_proc_phyReg=0;

//#define DBG_ACCESS_CPU_REGISTER
#ifdef DBG_ACCESS_CPU_REGISTER
static u32 switch_proc_cpuAddr=0x18050000;
static int switch_cpuRegAddr_show(char *page, char **start, off_t off,
...
static int switch_cpuRegAddr_store(struct file *file, const char __user *buf,
			unsigned long count, void *data)
{
	...
	switch_proc_cpuAddr = simple_strtoul(num, NULL, 0);
	return count;
}
static int switch_cpuRegVal_show(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	int len;
	void __iomem *base;

	base = ioremap_nocache(switch_proc_cpuAddr&0xffffff00, 0x100);
	len = sprintf(page, "0x%08X\n", __raw_readl(base+(switch_proc_cpuAddr&0xff)));
	iounmap(base);
	*eof = 1;
	return len;
}
static int switch_cpuRegVal_store(struct file *file, const char __user *buf,
			unsigned long count, void *data)
{
	char num[11];
	void __iomem *base;
	...
	val = simple_strtoul(num, NULL, 0);
	base = ioremap_nocache(switch_proc_cpuAddr&0xffffff00, 0x100);
	__raw_writel(val, base+(switch_proc_cpuAddr&0xff));
	iounmap(base);
	return count;
}
#endif

static int switch_phyRegAddr_show(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	int len;
	len = sprintf(page, "%d 0x%04X\n", switch_proc_phyAddr, switch_proc_phyReg);
	*eof = 1;
	return len;
}

static int switch_phyRegAddr_store(struct file *file, const char __user *buf,
			unsigned long count, void *data)
{
	char num[12], *ptr;
	int addr, reg;
	...
	addr = simple_strtoul(num, &ptr, 0);
	if (!isspace(*ptr) || addr >= 32 || addr < 0) {
		printk("Wrong %s!", addr>=32?"PHY address":"format");
		return -EINVAL;
	}
	ptr++;
	reg = simple_strtoul(ptr, NULL, 0);
	if ( reg < 0 || reg > 0xffff ) {
		return -EINVAL;
	}
	switch_proc_phyAddr = addr;
	switch_proc_phyReg  = reg;
	return count;
}

static int switch_phyRegVal_show(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	/* Read PHY register via MDIO/MDC bus */
	int len;
	u16 value;
	struct mii_bus *bus;
	...
	if (ar8216_priv_backup==NULL) {
		...
		bus = ar8216_priv_backup->phy->bus;
		mutex_lock(&bus->mdio_lock);
		value = bus->read(bus, switch_proc_phyAddr, switch_proc_phyReg);
...
static int switch_phyRegVal_store(struct file *file, const char __user *buf,
			unsigned long count, void *data)
{
	/* Write PHY register via MDIO/MDC bus */
	int ret = -EACCES;
	struct mii_bus *bus;
	char num[7];
	u16 value;
	...
	if (ar8216_priv_backup!=NULL) {
		...
		bus = ar8216_priv_backup->phy->bus;
		mutex_lock(&bus->mdio_lock);
		bus->write(bus, switch_proc_phyAddr, switch_proc_phyReg, value);
		mutex_unlock(&bus->mdio_lock);
		ret = count;
	}
	return ret;
...
static void ar8216_switch_proc_remove(void)
{
	if (switch_proc) {
		...
		remove_proc_entry(SWITCH_PROC_ENTRY_PHY_REG_ADDR, switch_proc);
		remove_proc_entry(SWITCH_PROC_ENTRY_PHY_REG_VAL, switch_proc);
#ifdef DBG_ACCESS_CPU_REGISTER
		remove_proc_entry(SWITCH_PROC_ENTRY_CPU_REG_ADDR, switch_proc);
		remove_proc_entry(SWITCH_PROC_ENTRY_CPU_REG_VAL, switch_proc);
#endif
...
static int ar8216_switch_proc_create(const char *chipName)
{
	...
	ent = create_proc_entry(SWITCH_PROC_ENTRY_PHY_REG_ADDR, 
			S_IFREG|S_IRUSR|S_IWUSR, switch_proc);
	if (!ent) { 
		printk("%s: Fail to create proc file '%s'!\n", __func__, 
			SWITCH_PROC_ENTRY_PHY_REG_ADDR);
		goto ar8216_switch_proc_create_failed4;
	} else {
		ent->read_proc = switch_phyRegAddr_show;
		ent->write_proc = switch_phyRegAddr_store;
	}
	ent = create_proc_entry(SWITCH_PROC_ENTRY_PHY_REG_VAL, 
			S_IFREG|S_IRUSR|S_IWUSR, switch_proc);
	if (!ent) { 
		printk("%s: Fail to create proc file '%s'!\n", __func__, 
			SWITCH_PROC_ENTRY_PHY_REG_VAL);
		goto ar8216_switch_proc_create_failed5;
	} else {
		ent->read_proc = switch_phyRegVal_show;
		ent->write_proc = switch_phyRegVal_store;
	}
#ifdef DBG_ACCESS_CPU_REGISTER
	ent = create_proc_entry(SWITCH_PROC_ENTRY_CPU_REG_ADDR, 
			S_IFREG|S_IRUSR|S_IWUSR, switch_proc);
	if (!ent) { 
		printk("%s: Fail to create proc file '%s'!\n", __func__, 
			SWITCH_PROC_ENTRY_CPU_REG_ADDR);
		goto ar8216_switch_proc_create_failed5;
	} else {
		ent->read_proc = switch_cpuRegAddr_show;
		ent->write_proc = switch_cpuRegAddr_store;
	}
	ent = create_proc_entry(SWITCH_PROC_ENTRY_CPU_REG_VAL, 
			S_IFREG|S_IRUSR|S_IWUSR, switch_proc);
	if (!ent) { 
		printk("%s: Fail to create proc file '%s'!\n", __func__, 
			SWITCH_PROC_ENTRY_CPU_REG_VAL);
		remove_proc_entry(SWITCH_PROC_ENTRY_CPU_REG_ADDR, switch_proc);
		goto ar8216_switch_proc_create_failed5;
	} else {
		ent->read_proc = switch_cpuRegVal_show;
		ent->write_proc = switch_cpuRegVal_store;
	}
#endif
	return 0;

ar8216_switch_proc_create_failed5:
	remove_proc_entry(SWITCH_PROC_ENTRY_PHY_REG_ADDR, switch_proc);
ar8216_switch_proc_create_failed4:
	remove_proc_entry(SWITCH_PROC_ENTRY_REGISTER_VAL, switch_proc);
...
ar8327_get_pad_cfg(struct ar8327_pad_cfg *cfg)
{
	...
	#if 1// should be 0? need to modify mach-ap135.c
		if (cfg->sgmii_delay_en)
			t |= AR8327_PAD_SGMII_DELAY_EN;
	#endif
		t |= cfg->sgmii_clk_phase_sel << AR8327_PAD_SGMII_CLK_PHASE_SEL_S;
		break;
...
ar8327_hw_init(struct ar8216_priv *priv)
{
	...
	if ( (pdata->pad0_cfg->mode==AR8327_PAD_MAC_SGMII) || 
		...
		/* due to AR8337 disable SGMII interface by default(power-on), 
                 * so we need to enable SGMII interface */
		priv->write(priv, AR8327_REG_SGMII_CTRL, 
			priv->read(priv, AR8327_REG_SGMII_CTRL)|0x0e);
	}
